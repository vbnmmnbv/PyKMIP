...
class TestKmipEngine(testtools.TestCase):
    ...
    def test_revoke(self):
        ...
        # Test that reason KEY_COMPROMISE will put object into state
        # COMPROMISED
        payload = revoke.RevokeRequestPayload(
            unique_identifier=attributes.UniqueIdentifier(object_id),
    ...
    def test_revoke_on_static_object(self):
        """
        Test that the right error is generated when an reovke request is
    ...
    def test_mac_crypto_usage_mask_not_set(self):
        ...
        args = (payload,)
        regex = "MAC GERERATE crypto usage mask must be set for the MAC " \
                "operation"
        self.assertRaisesRegexp(
            exceptions.PermissionDenied,
            regex,
